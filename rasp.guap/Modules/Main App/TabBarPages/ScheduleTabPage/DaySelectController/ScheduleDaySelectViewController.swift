...
class ScheduleDaySelectViewController: ViewController<ScheduleDaySelectView> {
    ...
    override func viewDidLoad() {
        ...
        self.rootView.weekBtn.addTarget(action: { (_) in
            if self.week == .odd {
                self.week = .even
            } else {
                self.week = .odd
	...
	func getIndex(day: Int) -> Int {
		...
		if day >= 0 {
			...
			for (index, name) in shouldShowDays.enumerated() where name == dayname {
	...
	private func getWeekDay(index: Int) -> Int {
		...
		if index >= 0 && index < shouldShowDays.count {
			for (day, name) in self.days.enumerated() where name == shouldShowDays[index].element {
